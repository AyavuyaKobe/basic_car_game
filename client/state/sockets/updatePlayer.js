...
const updatePlayers = (socket, otherPlayers, game) => {
    socket.on('update-players', playersData => {
        ...
        for(let index in playersData){
            ...
            if(otherPlayers[index] === underfined && index !== socket.id){
                ...
                newPlayer.updatePlayersName(data.playerName.name, data.playerName.x, data.playerName.y)
                
                otherPlayers[index] = newPlayer
            }

            playerFound[index] = true
            //update players data
            if(index !== socket.id){
                //update players target but not their real position
                otherPlayers[index].target_x = data.x
                otherPlayers[index].target_y = data.y
                otherPlayers[index].target_ratation = data.angle
